...
//        // App::call()->session->checkSessionAndDestroy();
    ...
    {
        $sql = "SELECT * FROM fr_main_table 
        WHERE (fr_main_table.fr_date >=:frDateOne 
        AND fr_main_table.fr_date <=:frDateTwo)
        OR fr_main_table.fr_date IS NULL
    ...
    {
        $sql = "SELECT * FROM fr_main_table 
        INNER JOIN nbs_users ON nbs_users.id =  fr_main_table.answering_id 
        INNER JOIN fr_scripts ON fr_scripts.script_id =  fr_main_table.fr_script_id 
        INNER JOIN fr_status_values ON fr_status_values.fr_status_id =  fr_main_table.fr_status 
        WHERE (fr_main_table.fr_date >=:frDateOne 
        AND fr_main_table.fr_date <=:frDateTwo) OR fr_main_table.fr_date IS NULL
        ";
        $params = [
            'frDateOne' => $frDateOne,
            'frDateTwo' => $frDateTwo
    ...
    {
        $sql = "SELECT *, fr_main_table.priority as priority_fr, (SELECT COUNT(*) FROM nbs_quote WHERE nbs_quote.proj_id = fr_main_table.proj_id ) as countQuote FROM fr_main_table 
        INNER JOIN nbs_users ON nbs_users.id =  fr_main_table.answering_id 
        INNER JOIN fr_scripts ON fr_scripts.script_id =  fr_main_table.fr_script_id 
        INNER JOIN fr_status_values ON fr_status_values.fr_status_id =  fr_main_table.fr_status 
        WHERE fr_main_table.fr_date >=:frDateOne
        AND fr_main_table.fr_date <=:frDateTwo
        -- AND fr_main_table.fr_status NOT IN (3,4,5,6)
        AND fr_main_table.status_client NOT IN (27,28,29,33)    
         -- ORDER by
    ...
    public function getAllbyDateDASHBORDOne($proj_id)
    {
        ...
        INNER JOIN fr_scripts ON fr_scripts.script_id =  fr_main_table.fr_script_id 
        INNER JOIN fr_status_values ON fr_status_values.fr_status_id =  fr_main_table.fr_status 
        WHERE fr_main_table.proj_id >=:proj_id
        ";
        $params = [
            'proj_id' => $proj_id
        ];
    ...
    {
        $sql = "SELECT nbs_users.id, COUNT(*) as total, CONCAT(nbs_users.lasttname, ' ', nbs_users.firstname) as fio FROM fr_main_table 
        INNER JOIN nbs_users ON nbs_users.id =  fr_main_table.answering_id 
        INNER JOIN fr_scripts ON fr_scripts.script_id =  fr_main_table.fr_script_id 
        INNER JOIN fr_status_values ON fr_status_values.fr_status_id =  fr_main_table.fr_status 
        WHERE fr_main_table.fr_date >=:frDateOne 
    ...
    public function getStaticGood($frDateOne)
    {
        ...
        INNER JOIN fr_scripts ON fr_scripts.script_id =  fr_main_table.fr_script_id 
        INNER JOIN fr_status_values ON fr_status_values.fr_status_id =  fr_main_table.fr_status 
        INNER JOIN nbs_orders_status_action ON nbs_orders_status_action.proj_id =  fr_main_table.proj_id 
        WHERE fr_main_table.fr_date >=:frDateOne 
    ...
    {
        $sql = " SELECT  proj_id, internal_id from fr_main_table  
        WHERE fr_main_table.fr_date >='2020-01-01'
        AND fr_status = 0";
    ...
    public function getForQuotesTable()
    {
        $sql = "SELECT
                ...
                fmt.history_quotes_search, fmt.clinical_inclusion, fmt.project_details, fmt.feas_russian
            FROM fr_main_table fmt
    ...
    public function getOrdersbyDateDeadLine ($dateOne, $dateTwo)
    {

        $sql = "SELECT * 
            FROM fr_main_table fmt
            INNER JOIN fr_scripts ON fr_scripts.script_id = fmt.fr_script_id
            WHERE fmt.deadline >= :dateOne
            AND fmt.deadline <= :dateTwo
            AND (fmt.status_manager = 2
            OR fmt.status_manager = 3)
           --  AND fmt.status_client != 28
            ORDER by fmt.deadline ASC
                ";
    ...
    public function getByScriptId($script_id)
    {
        $sql = "SELECT 
            ...
            project_details
            FROM fr_main_table fmt
            WHERE fmt.fr_script_id = :script_id
            ORDER by fmt.fr_date ASC
                ";
    ...
    public function getByScriptIdLimit($script_id, $date)
    {
        $sql = "SELECT 
            proj_id,
            fr_date,
    ...
    public function getByScriptByProj_id($proj_id,$fr_script_id)
    {
        $sql = "SELECT 
            proj_id,
            fr_date,
            ...
            project_details
            FROM fr_main_table fmt
            WHERE fmt.proj_id = :proj_id
            AND fmt.fr_script_id = :fr_script_id
            ORDER by fmt.fr_date ASC
                ";
