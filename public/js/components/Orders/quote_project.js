Vue.component('offer', {
    ...
    props: ['proj_id', 'script_id', 'company_users', 'currency', 'order_diseases', 'order_samples'],

    computed:{
        sample_options(){
            return this.order_samples.map(s => {
                return {
                    text: s.biospecimen_type + ' ' + s.modification,
    ...
    methods: {
        getOffer() {
            return new Promise(resolve => {
                this.$parent.getJson(`../../offer/getbyId/?proj_id=${this.proj_id}`)
                    .then(data => {
        ...
        getOfferItems(offer_id) {
            return new Promise(resolve => {
                this.$parent.getJson(`../../offerItem/getByOfferId/?offer_id=${offer_id}`)
                    .then(data => {
                        resolve(data);
                    });
            });
        ...
        async importOffer() {
            ...
            if (data.result.length) {
                for (let index in this.descriptionArray) {
                    ...
                    let clinicalInformationArray = await this.getClinicalInformation(offer_item_id);
                    ...
                    for (let i = 0; i < timelinesArray.length; i++) {
                        ...
                        // if (['', null, undefined].indexOf(timeline) !== -1) {
                        //     if (i === 1) {
            ...
            if (action == '+') {
                ...
                setTimeout(() => {
                    this.order_samples.forEach(order_sample => {
                        ...
                        //     $(`#samplesid_${this.descriptionRow - 1}`).append(`
                        //         <option value="${order_sample.biospecimen_type_id}"
                        //             id="${order_sample.disease_id}_${order_sample.biospecimen_type_id}"
                        //             modification="${order_sample.modification}">
                        //                 ${order_sample.biospecimen_type} ${order_sample.modification}
                        //         </option>`)
                        //
                        // }
                    })
                    let sample_id = $(`#samplesid_${this.descriptionRow - 2}`).val(),
                        ...
                        Mod = this.order_samples.filter(os => {
                            return parseInt(os.disease_id) === parseInt(id) &&
        ...
        samplesOptions($event) {
            // let val = $($event.currentTarget).val(), option = $(`#order_diseases [value="${val}"]`),
            //     id = option.data('id')
            let id = $("#order_diseases option:selected").attr('data-id')
            $(".order_samples").empty()
            this.order_samples.forEach(order_sample => {
