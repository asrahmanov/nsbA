Vue.component('worksheets', {
    data() {
        return {
            url: '/worksheets/getMy',
            worksheets: [],
            filteredWorksheets: [],
    ...
    methods: {
        ...
        openModal(proj_id) {
            ...
            window.open(
                'worksheets/quote/?id=' + element.worksheets_id,
        ...
        filter(status = '') {
            this.tableText = this.getName(status);
            this.internal_id = '';
        ...
        getName(name) {
            if (this.languages == 'en') {
                if(name == 'Новые' || name == 'Новый') {
                    return 'New'
                } else if (name == 'Обработан') {
                    return 'Processed'
                } else if (name == 'Отказ') {
                    return 'Denial'
                }
...
<button class="btn btn-outline-black" @click="filter('Новый')">{{ getName('Новые') }}</button>
<button class="btn btn-outline-info ml-1" @click="filter('Обработан')">{{ getName('Обработан') }}</button>
<button class="btn btn-outline-danger ml-1" @click="filter('Отказ')">{{ getName('Отказ') }}</button>
</div>
</div>
   <h2>{{ tableText }}</h2>
...
<td colspan="8"><img src="../../app-assets/img/loaders/rolling.gif" alt="loading" ></td>
    ...
    <td class="mobile-none">{{ getName(el.worksheets_status) }}</td>
